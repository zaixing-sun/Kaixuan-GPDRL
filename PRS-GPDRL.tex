...
\begin{abstract}
Learning an effective scheduling policy for multi-objective dynamic workflow scheduling requires online resource allocation to adapt to workflow and resource states and to the requested trade-off between weighted tardiness and energy consumption. This is challenging in heterogeneous cloud-edge systems because future workflow arrivals are unknown, feasible resources vary over time, and each allocation can affect successor release times, the progress of running tasks under CPU sharing, and resource active duration. Genetic programming (GP) hyper-heuristics can evolve scheduling heuristics, but the obtained heuristics are usually applied as fixed policies. Deep reinforcement learning supports adaptive rule selection, but using feasible resources as actions leads to a time-varying action set. This paper proposes Preference-Aware Risk-Stratified GP-assisted Deep Reinforcement Learning (PRS-GPDRL). Cooperative coevolution genetic programming (CCGP) first evolves complete scheduling heuristics with task-sequencing and resource-selection rules, and a quality- and behavior-aware procedure extracts a compact pool of complementary resource-selection rules. Each rule is paired with base and risk modes to form a fixed rule-mode action space, where the risk mode applies bounded preference-aware local correction. A preference-aware multi-objective double deep Q-network selects rule-mode actions using feature-wise linear modulation and augmented weighted Tchebycheff scalarization. Experimental results demonstrate that PRS-GPDRL obtains the best average ranks of 1.406 for hypervolume and 1.486 for inverted generational distance, and is statistically better than or comparable to CCGP in 11 of 12 scenarios for each metric. Further analyses and ablation results show preference-dependent policy behavior and identify risk-mode correction and GP-evolved resource selection as the main sources of improvement.
\end{abstract}
...
\begin{itemize}
    \item We establish a dynamic workflow scheduling model for heterogeneous Cloud--Edge systems with total weighted tardiness and energy consumption as the two objectives. The model jointly considers online workflow arrivals, workflow weights and deadlines, heterogeneous Edge and Cloud resources, and runtime CPU sharing among tasks running on the same resource.

    \item We propose PRS-GPDRL, a preference-aware GP-assisted DRL framework for online resource selection. CCGP first evolves complete scheduling heuristics, and a quality- and behavior-aware procedure selects a compact pool of complementary resource-selection rules. These rules form the rule component of the DDQN action space, so the learned policy selects among evolved resource-selection rules instead of defining actions directly on the time-varying set of feasible resources.

    \item We develop risk-stratified action augmentation and preference-aware multi-objective DDQN learning. The base mode follows GP rankings, while the risk mode adds bounded local correction. Feature-wise linear modulation, vector-valued action costs, and augmented weighted Tchebycheff scalarization allow the DDQN to select rule-mode actions under different preferences.

    \item Experimental results show that PRS-GPDRL obtains the best average ranks of 1.406 for hypervolume and 1.486 for inverted generational distance, and is statistically better than or comparable to CCGP in 11 of 12 scenarios for each metric. Further analyses and ablation results support the roles of preference-aware rule selection, risk-mode correction, and GP-evolved resource selection.
\end{itemize}
...
For each scenario, the resource-selection rule pool extracted from the CCGP archive and the corresponding rule-mode action space are fixed before DDQN training and then used unchanged in validation and testing. The DDQN learns from vector-cost transitions under sampled preferences using Eq.~\eqref{Eq: tchebycheff scalarization}, and the trained DDQN together with the fixed action space constitutes the PRS-GPDRL agent. During inference, upward-rank sequencing resolves simultaneous ready tasks when needed, after which the DDQN selects a rule-mode action and assigns the current task to the feasible resource with the minimum priority score.
...
\begin{table}[!t]
\caption{Parameter settings of CCGP \cite{ccgp}.}
...
For each scenario and metric, statistical comparisons are performed using the Friedman test and two-sided paired Wilcoxon signed-rank tests at the 0.05 significance level \cite{ccgp}. In the result tables, the arrows indicate that a result is statistically better ($\uparrow$), worse ($\downarrow$), or similar ($\approx$) compared with the preceding methods. The win/draw/loss (W/D/L) row summarizes the number of scenarios in which the reported method is statistically better than, similar to, or worse than the reference method in the corresponding comparison. AverageRank reports the mean rank over the 30 runs, and a smaller value is better.
...
The CCGP parameters are listed in Table~\ref{Table: CCGP parameters}. GATES uses a population of 100 policy networks and runs for 51 generations. The main DDQN and risk-stratified action augmentation parameters of PRS-GPDRL are listed in Table~\ref{Table: DDQN risk parameters}. For the diagnostic study, CCGP with upward-rank task sequencing (CCGP-UR) retains the GP resource-selection rule, whereas CCGP with earliest-finish-time (EFT) resource selection (CCGP-EFT) retains the GP task-sequencing rule; both replacement components follow the Heterogeneous Earliest Finish Time (HEFT) algorithm \cite{Topcuoglu2002}.
...
& \multicolumn{4}{c}{HV} & \multicolumn{4}{c}{IGD} \\
\cmidrule(lr){2-5}\cmidrule(lr){6-9}
& GATES \cite{gates} & LWFF \cite{lwff} & CCGP \cite{ccgp} & PRS-GPDRL
& GATES \cite{gates} & LWFF \cite{lwff} & CCGP \cite{ccgp} & PRS-GPDRL \\
\midrule
$\langle10,0.2\rangle$
