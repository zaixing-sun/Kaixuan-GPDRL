...
Fig.~\ref{Fig: PRS-GPDRL framework} shows the overall framework of PRS-GPDRL, which consists of offline training and online scheduling. For each training scenario, CCGP first produces an archive of complete scheduling heuristics, each of which contains a task-sequencing rule and a resource-selection rule. Although both rules are evolved cooperatively, PRS-GPDRL uses the resource-selection rules to construct the DDQN action space and uses upward-rank sequencing for ready-task sequencing. The task-sequencing ablation experiment in Section~\ref{Sec: task sequencing design} compares upward-rank sequencing with the GP task-sequencing rule while fixing the resource-selection rule pool. The results show that upward-rank sequencing is statistically comparable to the GP task-sequencing rule in 11 of 12 scenarios. Resource selection is required for every dispatched task, whereas task sequencing is needed only when multiple tasks are ready simultaneously. Therefore, PRS-GPDRL fixes upward-rank sequencing and focuses the learned action space on resource-selection rules and their modes.
...
By comparison, the \emph{w/o FiLM} variant records 0/3/3 for HV and 0/4/2 for IGD and
...
evaluation. The mixture of statistically comparable outcomes and losses
suggests that FiLM is not the only way in which preference information enters
...
\subsubsection{Task-Sequencing and Resource-Selection Ablation}
\label{Sec: task sequencing design}

Table~\ref{Table: combined GP task sequencing} reports two diagnostic comparisons
across the 12 scenarios. CCGP-UR replaces the GP task-sequencing rule of CCGP
with upward-rank sequencing while retaining the GP resource-selection rule,
whereas CCGP-EFT retains the GP task-sequencing rule but replaces the GP
resource-selection rule with earliest-finish-time (EFT) resource selection.
...
CCGP-UR records 0/11/1 against CCGP for both metrics, with similar average
ranks, indicating that the GP task-sequencing rule and upward-rank sequencing are
statistically comparable in 11 of 12 scenarios. In contrast, CCGP-EFT records
0/0/12 against CCGP for both metrics and yields substantially worse performance.
Within the evaluated scenarios, this contrast indicates that the
resource-selection design is more consequential than replacing the GP
task-sequencing rule with upward-rank sequencing.

This pattern is consistent with the decision structure considered here: VMs can
execute tasks concurrently, resource selection is required for every dispatched
task, and task sequencing is needed only when multiple tasks become ready at the
same decision point. Task-sequencing decisions are relatively sparse, so each
scheduling episode provides fewer task-sequencing decisions than resource-selection
decisions. Their effects on the two objectives are also realized through later
resource allocations and downstream workflow progress, which makes credit
assignment for task-sequencing learning less direct. Learning task sequencing as
well would introduce an additional offline learning process and further increase
training cost beyond the current resource-selection learning process. These
results support the design
choice in Section~\ref{Sec: PRS-GPDRL overall}: PRS-GPDRL retains upward-rank
sequencing for ready-task sequencing and concentrates learning on resource
selection.
...
PRS-GPDRL also obtains the best IGD average rank (1.486), with CCGP recording
...
explained simply by replacing the GP task-sequencing rule or by increasing the
...
sequencing provides performance comparable to GP-based task sequencing in most
scenarios.
