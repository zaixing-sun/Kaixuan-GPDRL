...
Fig.~\ref{Fig: PRS-GPDRL framework} shows the overall framework of PRS-GPDRL, which consists of offline training and online scheduling. For each training scenario, CCGP first produces an archive of complete scheduling heuristics, each of which contains a task-selection rule and a resource-selection rule. Although both rules are evolved cooperatively, PRS-GPDRL uses the resource-selection rules to construct the DDQN action space and uses upward-rank ordering for ready-task sequencing. The task-ordering ablation experiment in Section~\ref{Sec: task ordering design} compares upward-rank ordering with the GP task-selection rule while fixing the resource-selection rule pool. The results show that upward-rank ordering is statistically comparable to the GP task-selection rule in 11 of 12 scenarios. Resource selection is required for every dispatched task, whereas task ordering is needed only when multiple tasks are ready simultaneously. Therefore, PRS-GPDRL fixes upward-rank ordering and focuses the learned action space on resource-selection rules and their modes.
...
\textbf{CCGP training.} CCGP cooperatively evolves task-selection and resource-selection rules in two subpopulations. Individuals from the two subpopulations are paired to form complete scheduling heuristics, which are evaluated on energy consumption and weighted tardiness and recorded in the archive set ($AS$). Until the stopping criterion is met, CCGP updates the two subpopulations by elitism, reproduction, crossover, and mutation. At the final generation, duplicate solutions are removed from $AS$, and the first non-dominated front is retained as the source of resource-selection rules for PRS-GPDRL.

\textbf{Rule representation.} Each GP rule is a tree-based priority expression whose terminals provide scheduling attributes and whose function nodes combine them into priority scores. Task- and resource-selection rules use different terminal subsets but share the same function set. Table~\ref{Table:scheduling-attributes} summarizes the terminals used by the GP rules and the attributes used by the DDQN state encoder. The DDQN state uses task, feasible-resource, and preference attributes, so its input is consistent with the information used by the rule-mode actions. Further details of CCGP training and terminal-value computation are given in~\cite{ccgp}.
...
endpoints, their neighboring regions, and the balanced anchor, and obtains the best
...
the main explanation for the overall gain over CCGP. This pattern is consistent
with the decision structure considered here: VMs can execute tasks concurrently,
resource selection is required for every dispatched task, and task ordering is
needed only when multiple tasks become ready at the same decision point.
Task-ordering decisions are relatively sparse, so each scheduling episode
provides fewer task-selection transitions than resource-selection transitions.
Their effects on the two objectives are also realized through later resource
allocations and downstream workflow progress, which makes credit assignment for
task-selection learning less direct. Learning an additional task-selection
policy would introduce many combinations of task-selection and
resource-selection rules and require extra simulation and optimization beyond
the current resource-selection learning process. Given the comparable
performance observed with the resource-selection rule pool fixed, these results
support the design choice in Section~\ref{Sec: PRS-GPDRL overall}: PRS-GPDRL
retains upward-rank ordering for ready-task sequencing and concentrates learning
...
energy-oriented roles through the balanced anchor toward tardiness-oriented roles.
...
the resource ranking by moving from energy-oriented roles through the balanced anchor
...
the best overall HV and IGD performance among the compared methods. The
behavioral and ablation analyses further confirm that complementary GP rules,
preference conditioning, and state-dependent local correction jointly
contribute to its adaptive scheduling performance, while fixed upward-rank
ordering provides performance comparable to GP-based task ordering in most
scenarios.

The current framework still relies on an offline fixed rule pool, fixed
upward-rank task ordering, and a predefined CPU-sharing mechanism. Future work
will investigate continual GP--DRL integration to update the rule pool during
deployment, hierarchical or multi-timescale learning for adaptive task ordering
