...
This section describes the simulation setup, compared algorithms, evaluation metrics, and statistical tests used in the experiments.
...
types can be provisioned on demand. Each task incurs a 50ms container
...
Workflow templates are sampled from Alibaba Cluster-trace-v2018\footnotemark[3]. The public trace
...
finish time obtained by this propagation. The workflow deadline is
...
\subsection{Compared Algorithms and Performance Measures}
...
\begin{itemize}
    ...
    performs individual selection, and offspring networks are generated by
    Gaussian perturbations of the selected network parameters. The final
    non-dominated policies form the approximation set.
    ...
    finish time resolves ties. Applying LWFF over the evaluation preference
    grid and retaining the non-dominated schedules produces its approximation
    set.
\end{itemize}
...
arrows denote statistically better ($\uparrow$), worse ($\downarrow$), or
similar ($\approx$) performance against the preceding methods. For each
baseline column, W/D/L (win/draw/loss) gives the number of scenarios in which
that baseline is statistically better than, similar to, or worse than
PRS-GPDRL. The PRS-GPDRL column is marked as not applicable (N/A). AverageRank
is the mean rank over the 30 runs, for which smaller is better.

\section{Experimental Results}
\label{Sec: experimental results}

This section first reports the ablation study of PRS-GPDRL and then compares its overall performance with the baselines.
...
\emph{w/o Risk} and \emph{w/o FiLM} variants remove the two components
separately, while $K=3$ sets the maximum requested resource-selection rule pool
size to three. All variants follow the same training, evaluation, and
...
PRS-GPDRL for both metrics, the strongest ablation effect. A retained GP rule
provides a global resource ranking, while the Risk mode applies a bounded
priority correction using the specified preference, task urgency, CPU sharing,
resource active time, and Cloud provisioning information. The consistent result
across these six scenarios supports the role of state- and
preference-dependent local correction in the full method.
...
the policy. Its additional preference-dependent modulation of the shared state
representation is consistent with complementary improvements in trade-off
...
five of six scenarios. This indicates that the main capability does not arise
simply from increasing the number of available actions, since a small set of
behaviorally complementary rules preserves most of the performance. Within the
tested settings, performance is reasonably robust to pool size when
behaviorally diverse rule roles are retained. The $K=5$ design targets both
endpoints, their neighboring regions, and the knee, and obtains the best
average ranks. The comparison supports the motivation for quality- and
behavior-aware filtering: coverage of complementary global ranking roles is
more important than rule count alone.

The ablations organize the main components into distinct roles. Risk provides
state- and preference-dependent local correction, FiLM adds
preference-dependent modulation of the shared representation, and the
resource-selection rule pool provides complementary global ranking coverage.
These roles are complementary within the proposed decision process.

\subsubsection{Task-Ordering Ablation}
\label{Sec: task ordering design}

Table~\ref{Table: combined GP task ordering} reports the task-ordering
ablation results across the 12 scenarios. The comparison holds the
deduplicated resource-selection rule pool fixed and compares the GP
task-selection rule with upward-rank ordering. W/D/L reports the baseline GP
...
ranks. Upward-rank ordering is statistically comparable in 11 of 12 scenarios.
These results support the use of fixed upward-rank ordering in PRS-GPDRL. In
the considered model, VMs can execute tasks concurrently, and task ordering is
needed only when multiple tasks become ready at the same decision point,
whereas resource selection is needed for every dispatched task. Focusing the
action space on resource selection avoids many task-rule and resource-rule
combinations without sacrificing solution quality in most scenarios.

The ablation results provide context for interpreting the baseline comparison.
They indicate that the performance gains should be interpreted mainly through
the learned use of complementary resource-selection rules and the bounded Risk
correction, rather than through rule-pool size or the replacement of the GP
task-selection rule with upward-rank ordering.
...
rules. The ablation results above further show that the comparison is not
explained simply by replacing the GP task-selection rule or by increasing the
number of retained rules. These observations motivate a closer analysis of the
obtained trade-off profiles and the policy behavior under different
preferences.

\section{Further Analysis}
\label{Sec: further analysis}

This section further analyzes the obtained trade-off profiles and the behavior of the learned policy. The analysis focuses on whether PRS-GPDRL improves the distribution of nondominated solutions and how the selected resource-selection rules and Risk mode change with the requested preference.
...
the endpoints. These results show that one network produces preference-dependent scheduling
...
of included runs shift as expected. This pattern is consistent with
Fig.~\ref{Fig: preference responses}: increasing $w_T$ moves selection from
energy-oriented roles through the knee toward tardiness-oriented roles.
...
base-mode decisions indicate that local correction is not always required. The
policy adaptively chooses between using the
...
distinct levels of adaptation. Rule selection changes the main orientation of
the resource ranking by moving from energy-oriented roles through the knee
toward tardiness-oriented roles as $w_T$ increases. Risk-mode selection instead
determines whether the current decision requires a bounded local modification
of that ranking. Its frequency need not vary monotonically with $w_T$, because
the need for correction depends jointly on the requested preference, task
urgency, CPU sharing, resource active time, Cloud provisioning, and the current
feasible-resource state. This frequency reflects the conditional use of local
correction under the current state and preference rather than serving as a
direct proxy for the preference weight.

Within the evaluated preference grid, the three figures describe policy
behavior at different levels. Fig.~\ref{Fig: preference responses} shows the
objective response to preference changes. Fig.~\ref{Fig: tree role frequency}
shows the corresponding migration of global rule roles.
Fig.~\ref{Fig: risk frequency} shows whether local correction is selected at the
mode level. Taken together, these observations support the interpretation that
preference conditioning changes both the resulting objectives and the internal
decisions that produce them: which global ranking is used and whether it is
locally corrected for the current state.
