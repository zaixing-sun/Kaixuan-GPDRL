...
%         Hejiao~Huang,
%         Chonglin~Gu,
%         and~Yi~Mei,~\IEEEmembership{Senior Member,~IEEE,}
...
% \thanks{~~Zaixing Sun and Bin Wang are with the Department of New Networks, Pengcheng Laboratory, Shenzhen, 518066, China. E-mail: \nolinkurl{zaixing.sun@micc.hitsz.edu.cn}, wangb02@pcl.ac.cn.}
...
% \thanks{~~Yi Mei is with the Evolutionary Computation and Machine Learning Research Group, {Centre for Data Science and Artificial Intelligence \rm\&} School of Engineering and Computer Science, Victoria University of Wellington, Wellington 6140, New Zealand. E-mail: {yi.mei@ecs.vuw.ac.nz.}}
% }
...
\begin{abstract}
Learning an effective scheduling policy for multi-objective dynamic workflow scheduling requires online resource allocation to adapt to workflow and resource states and to the requested trade-off between weighted tardiness and energy consumption. In heterogeneous Cloud--Edge systems, this is challenging because future workflow arrivals are unknown, feasible resources vary over time, and each allocation can affect successor release times, the progress of running tasks under CPU sharing, and resource active duration. Genetic programming (GP) hyper-heuristics can evolve scheduling rules, but the obtained heuristics are usually applied as fixed policies. Deep reinforcement learning (DRL) can support adaptive rule selection, but directly using feasible resources as actions leads to a time-varying action set. This paper proposes Preference-Aware Risk-Stratified Genetic Programming-assisted Deep Reinforcement Learning (PRS-GPDRL). Cooperative coevolution genetic programming (CCGP) first evolves complete scheduling heuristics with task-sequencing and resource-selection rules, and a quality- and behavior-aware procedure extracts a compact pool of complementary resource-selection rules. Each rule is paired with a base mode and a risk mode to form a fixed rule-mode action space, where the risk mode applies bounded preference-aware local correction. A preference-aware multi-objective double deep Q-network selects rule-mode actions using feature-wise linear modulation and augmented weighted Tchebycheff scalarization. Experiments on 12 scenarios with 30 independent runs show that PRS-GPDRL obtains the best average ranks of 1.406 for hypervolume and 1.486 for inverted generational distance, and is statistically better than or comparable to CCGP in 11 of 12 scenarios for each metric. Further analyses and ablation results show preference-dependent policy behavior and identify risk-mode correction and GP-evolved resource selection as the main sources of improvement.
\end{abstract}
...
\begin{itemize}
    \item We establish a dynamic workflow scheduling model for heterogeneous Cloud--Edge systems with total weighted tardiness and energy consumption as the two objectives. The model jointly considers online workflow arrivals, workflow weights and deadlines, precedence and data dependencies, heterogeneous Edge and Cloud resources, and runtime CPU sharing among tasks running on the same resource.

    \item We propose PRS-GPDRL, a preference-aware framework that connects CCGP-based rule generation with DRL-based online resource selection. CCGP produces complete scheduling heuristics with task-sequencing and resource-selection rules, and a quality- and behavior-aware extraction procedure retains a compact pool of complementary resource-selection rules. The resulting action design enables the learned policy to select among GP-evolved resource-selection rules without treating the varying feasible-resource set as the DDQN action set.

    \item We develop risk-stratified action augmentation and preference-aware multi-objective DDQN learning for resource selection. Each retained rule is paired with a base mode and a risk mode. The risk mode keeps the normalized GP priority score as the main ranking term and adds a bounded correction based on tardiness-related and energy-related local costs under the requested preference. Feature-wise linear modulation (FiLM), vector-valued action costs, and augmented weighted Tchebycheff scalarization enable one DDQN to select rule-mode actions under different tardiness--energy preferences.

    \item We conduct extensive simulation experiments on 12 dynamic scheduling scenarios with 30 independent runs per scenario. PRS-GPDRL obtains the best average ranks for hypervolume (HV) and inverted generational distance (IGD), with ranks of 1.406 and 1.486, respectively, and is statistically better than or comparable to CCGP in 11 of 12 scenarios for each metric. Further analyses show that objective values, resource-selection rule usage, and risk-mode usage change with the requested preference. Ablation and diagnostic results further support the roles of risk-stratified correction, FiLM conditioning, rule-pool size, and GP-based resource selection.
\end{itemize}



    % \item We propose PRS-GPDRL, a preference-aware GP-assisted DRL framework for multi-objective dynamic workflow scheduling in Cloud--Edge computing. CCGP produces complete scheduling heuristics with task-sequencing and resource-selection rules, and a quality- and behavior-aware extraction procedure retains a compact pool of complementary resource-selection rules from the non-dominated archive. The learned policy selects among these GP-evolved rules without treating the varying feasible-resource set as the DDQN action set.

    % \item We develop risk-stratified rule-mode actions and preference-aware multi-objective DDQN learning for online resource selection. Each retained rule is paired with a base mode and a risk mode. The risk mode keeps the normalized GP priority score as the main ranking term and adds a bounded correction based on tardiness-related and energy-related local costs under the requested preference. Feature-wise linear modulation (FiLM), vector-valued action costs, and augmented weighted Tchebycheff scalarization enable one DDQN to select rule-mode actions under different tardiness--energy preferences.

    % \item We conduct extensive simulation experiments under a dynamic Cloud--Edge workflow model with online arrivals, workflow weights and deadlines, precedence and data dependencies, heterogeneous resources, and runtime CPU sharing. Across 12 scenarios with 30 independent runs per scenario, PRS-GPDRL obtains the best average ranks for hypervolume (HV) and inverted generational distance (IGD), with ranks of 1.406 and 1.486, respectively, and is statistically better than or comparable to CCGP in 11 of 12 scenarios for each metric.

    % \item We analyze the learned policy from objective outcomes, rule usage, and mode usage. The preference-response analysis shows that the obtained objective values change with the requested preference, while the rule-usage and risk-mode analyses show corresponding changes in internal decisions. Ablation and diagnostic comparisons further support the roles of risk-stratified correction, FiLM conditioning, rule-pool size, and GP-evolved resource selection.
...
The 128-dimensional embedding $\mathbf{z}_t$ is processed by a shared 256--128 rectified linear unit (ReLU) MLP and a linear output layer that produces a two-dimensional cost estimate for every rule-mode action:
\begin{equation}
...
The CCGP parameters follow \cite{ccgp} and are listed in Table~\ref{Table: CCGP parameters}. GATES uses a population of 100 policy networks and runs for 51 generations. The main DDQN and risk-stratified action augmentation parameters of PRS-GPDRL are listed in Table~\ref{Table: DDQN risk parameters}. For the diagnostic study, CCGP with upward-rank task sequencing (CCGP-UR) retains the GP resource-selection rule, whereas CCGP with earliest-finish-time (EFT) resource selection (CCGP-EFT) retains the GP task-sequencing rule; both replacement components follow the Heterogeneous Earliest Finish Time (HEFT) algorithm \cite{Topcuoglu2002}.
...
This diagnostic comparison examines how performance changes when either the task-sequencing or resource-selection component of CCGP is replaced by the corresponding HEFT component. Table~\ref{Table: combined GP task sequencing} reports the two CCGP-based diagnostic comparisons across the 12 scenarios.
...
\begin{figure*}[!t]
    ...
    each of the 12 scenarios under different tardiness preferences. Results
    include 332/360 scenario--run cases (92.2\%) retaining all five rules. The
    roles are the minimum-energy endpoint $g_1$, its
...
This paper studied preference-aware dynamic workflow scheduling in heterogeneous Cloud--Edge systems with total weighted tardiness and energy consumption as the two objectives. PRS-GPDRL combines CCGP-based rule discovery with DRL-based online resource selection. CCGP produces an archive of complete scheduling heuristics, and a quality- and behavior-aware procedure extracts a compact pool of complementary resource-selection rules from this archive. Pairing each retained rule with a base mode and a risk mode gives a fixed rule-mode action space for the preference-aware multi-objective DDQN. Upward-rank sequencing is used when multiple ready tasks need to be ordered, and the DDQN learns to select resource-selection rule-mode actions. The experimental results on 12 dynamic scheduling scenarios with 30 independent runs per scenario show that PRS-GPDRL achieves the best overall HV and IGD performance among the compared methods. It obtains average ranks of 1.406 for HV and 1.486 for IGD, and is statistically better than or comparable to CCGP in 11 of 12 scenarios for each metric. Further analyses show that the requested preference is reflected in the obtained objective values, selected resource-selection rules, and risk-mode usage. The ablation study shows that removing the risk mode causes the largest degradation among the tested variants, while FiLM provides additional preference-dependent modulation. The CCGP-EFT and CCGP-UR diagnostic comparison shows that replacing the GP resource-selection rule causes a much larger loss than replacing the GP task-sequencing rule with upward-rank sequencing in the evaluated scenarios. These results support the effectiveness of preference-aware online selection among GP-evolved resource-selection rules and bounded local correction.

The study has several limitations that suggest future research directions. The extracted resource-selection rule pool is fixed for each scenario after offline CCGP training, and the CPU allocation follows the predefined sharing rule in the problem model. In addition, PRS-GPDRL concentrates learning on resource selection, while task sequencing is handled by upward rank according to the diagnostic results. Future work will study adaptive updating of the rule pool and transfer across workload settings, joint learning of task sequencing and resource selection without substantially increasing the number of action combinations, and preference-aware CPU allocation policies that can be optimized together with scheduling decisions.
