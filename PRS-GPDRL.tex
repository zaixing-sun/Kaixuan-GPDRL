...
\begin{abstract}
Learning an effective scheduling policy for multi-objective dynamic workflow scheduling requires adapting online resource allocation to the current workflow and resource states and to the requested trade-off between weighted tardiness and energy consumption. In heterogeneous Cloud--Edge systems, this is challenging because future workflow arrivals are unknown, feasible resources vary with task requirements, resource availability, and Cloud provisioning, and an allocation can affect successor release times, the progress of running tasks, and resource active time. Genetic programming (GP) hyper-heuristics can automatically evolve scheduling rules for online decisions, but the evolved heuristics are usually applied as fixed policies. Deep reinforcement learning (DRL) can learn adaptive rule selection, yet directly using feasible resources as actions is difficult when the action set varies over time. This paper proposes Preference-Aware Risk-Stratified Genetic Programming-assisted Deep Reinforcement Learning (PRS-GPDRL). Cooperative coevolution genetic programming (CCGP) first evolves complete scheduling heuristics with task-sequencing and resource-selection rules. A quality- and behavior-aware extraction procedure then selects a compact pool of complementary resource-selection rules. Each retained rule is paired with a base mode and a risk mode to form a fixed rule-mode action space, and a multi-objective double deep Q-network selects rule-mode actions according to the current state and requested preference. Experimental results on 12 scenarios with 30 independent runs per scenario show that PRS-GPDRL obtains the best average ranks for hypervolume (HV) and inverted generational distance (IGD), with ranks of 1.406 and 1.486, respectively. PRS-GPDRL is statistically better than or comparable to CCGP in 11 of 12 scenarios for each metric. Further analyses show that objective values, resource-selection rule usage, and risk-mode usage change with the requested preference. Ablation and diagnostic results show the importance of risk-stratified local correction, preference conditioning, and GP-based resource selection.
\end{abstract}
